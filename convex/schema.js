...
exports.default = (0, server_1.defineSchema)({
    ...
    // Cost tracking
    cost_entries: (0, server_1.defineTable)({
        type: values_1.v.string(), // "llm", "compute", "storage"
        ...
        // Metadata
        tokens: values_1.v.optional(values_1.v.number()),
        requests: values_1.v.optional(values_1.v.number()),
        duration: values_1.v.optional(values_1.v.number()),
        bytes: values_1.v.optional(values_1.v.number()),
        package_name: values_1.v.optional(values_1.v.string()),
        job_id: values_1.v.optional(values_1.v.string()),
        // Timestamps
        timestamp: values_1.v.number(),
        date: values_1.v.string(), // YYYY-MM-DD for easy daily aggregation
        month: values_1.v.string(), // YYYY-MM for monthly aggregation
    }).index("by_timestamp", ["timestamp"])
        .index("by_date", ["date"])
    ...
    daily_costs: (0, server_1.defineTable)({
        date: values_1.v.string(), // YYYY-MM-DD
        total_cost: values_1.v.number(),
        llm_cost: values_1.v.number(),
        compute_cost: values_1.v.number(),
        storage_cost: values_1.v.number(),
        // Breakdown by provider
        cost_by_provider: values_1.v.object({
            openrouter: values_1.v.number(),
            local: values_1.v.number(),
            convex: values_1.v.number(),
        }),
        // Usage metrics
        total_requests: values_1.v.number(),
        total_tokens: values_1.v.number(),
        total_analyses: values_1.v.number(),
        created_at: values_1.v.number(),
        updated_at: values_1.v.number(),
    }).index("by_date", ["date"]),
    // Analytics events
    analytics_events: (0, server_1.defineTable)({
        event: values_1.v.string(),
        properties: values_1.v.any(),
        timestamp: values_1.v.number(),
        date: values_1.v.string(), // YYYY-MM-DD
        hour: values_1.v.string(), // YYYY-MM-DD-HH
        // Optional user tracking (for future use)
        user_id: values_1.v.optional(values_1.v.string()),
        session_id: values_1.v.optional(values_1.v.string()),
        // Environment context
        environment: values_1.v.string(),
        node_version: values_1.v.optional(values_1.v.string()),
    }).index("by_timestamp", ["timestamp"])
        .index("by_date", ["date"])
        .index("by_hour", ["hour"])
    ...
    system_metrics: (0, server_1.defineTable)({
        timestamp: values_1.v.number(),
        // Queue metrics
        queue_waiting: values_1.v.number(),
        queue_active: values_1.v.number(),
        queue_completed: values_1.v.number(),
        queue_failed: values_1.v.number(),
        // Performance metrics
        avg_response_time: values_1.v.number(),
        memory_usage_mb: values_1.v.number(),
        cpu_usage_percent: values_1.v.number(),
        // Cost metrics
        daily_cost: values_1.v.number(),
        monthly_cost: values_1.v.number(),
        // Analysis metrics
        success_rate: values_1.v.number(),
        throughput_per_hour: values_1.v.number(),
        // Cache metrics
        cache_hit_rate: values_1.v.number(),
        created_at: values_1.v.number(),
    }).index("by_timestamp", ["timestamp"]),
